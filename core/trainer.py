...
# from models.delg import DELG
from master_models.delg_model import DELG
from datasets.loader import _construct_train_loader, _construct_test_loader
from core.meters import topk_errors, setup_criterion
...
def setup_model():
    """model = DELG(backbone=cfg.MODEL.BACKBONE, num_classes=cfg.TRAIN.DATASET_NUM_CLASS)"""
    model = DELG()
...
def train_epoch(train_loader, model, optimizer, criterion, cur_epoch, print_freq=1, tb_logger=None):
    ...
    for ims, labels in logger.log_every(train_loader, print_freq, header):
        ...
        """
        _, _, _, local_expand_feats, _, \
        global_logits, local_attn_logits, \
        local_featmap, block3 = model(ims)  
          
        loss_dict = criterion(global_logits, local_attn_logits, local_expand_feats, block3, labels)
        
        global_loss = loss_dict['global_loss']
        ...
        optimizer.zero_grad()
        net.freeze_weights(model, freeze=['attn', 'auto_encoder', 'attn_head'])
        global_loss.backward()
        
        net.freeze_weights(model, freeze=['extractor', 'gem_pool', 'global_head'])
        net.unfreeze_weights(model, freeze=['attn', 'auto_encoder', 'attn_head'])
        attn_loss.backward(retain_graph=True)
        restruct_loss.backward()
        
        # restruct_loss.backward(retain_graph=True)
        # attn_loss.backward()
        
        net.unfreeze_weights(model, freeze=['extractor', 'gem_pool', 'global_head'])
        
        optimizer.step()
        """
        
        ims, labels = ims.cuda(), labels.cuda(non_blocking=True)
        # Perform the forward pass
        _, global_logits, _, local_logits, _ = model(ims, labels)
        # Compute the loss
        desc_loss = criterion(global_logits, labels)
        att_loss = criterion(local_logits, labels)
        loss_dict = {'desc_loss': desc_loss, 'att_loss': att_loss}
...
def train_model(args):
    ...
    artifact_name = (
        f'backbone_{cfg.MODEL.BACKBONE}_lr_{cfg.OPTIM.BASE_LR}'
    )
