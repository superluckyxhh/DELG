...
from core.meters import topk_errors, set_global_criterion, set_arcface_criterion
...
def setup_model():
    model = DELG_Global(cfg.MODEL.BACKBONE, cfg.TRAIN.DATASET_NUM_CLASS)
...
def train_epoch(train_loader, model, optimizer, criterion, cur_epoch, print_freq=1, tb_logger=None):
    ...
    for ims, labels in logger.log_every(train_loader, print_freq, header):
        ...
        global_prelogits, global_logits = model(ims)  
          
        global_loss = criterion(global_logits, labels)
        loss_dict['loss'] = global_loss
...
def train_global_model(args):
    ...
    """criterion = set_global_criterion()"""
    criterion = set_arcface_criterion()
    optimizer = optim.construct_optimizer(model)
    
    artifact_name = (
        f'global_backbone_{cfg.MODEL.BACKBONE}_lr_{cfg.OPTIM.BASE_LR}'
    )
...
def test_global_model(args, model_weights):
    setup_env(args)
    model = setup_model()
